...
exports.driverLogin = async (req, res) => {
    ...
    try {
        ...
        const token = jwt.sign({ id: driverId, role: 'driver', pozivnoy: pozivnoy }, process.env.JWT_SECRET, { expiresIn: '30d' });
        res.json({ success: true, token, driver: { id: driverId, name: driverData.name, phone: driverData.phone, car: driverData.car, plate: driverData.plate } });
    } catch (error) {
...
exports.clientLogin = async (req, res) => {
    ...
    try {
        ...
        const token = jwt.sign({ id: clientId, role: 'client' }, process.env.JWT_SECRET, { expiresIn: '30d' });
